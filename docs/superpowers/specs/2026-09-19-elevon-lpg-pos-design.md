...
| **5 Day close UI, customers UI, dashboard, reports** | `/day-close` (open, movements, close, reopen, Z-report), `/customers` (statement, receive payment), dashboard, report tabs, exports | §6.5, §6.7, §6.8. **DONE 2026-09-21** on `dev`, plan `docs/superpowers/plans/2026-09-20-phase-5-dashboard-reports.md` (tasks P1–P6). Deviations: the tax report's Tax excludes further tax, shown as its own line (a closed day's sealed `tax_collected` is still Tax + FurtherTax, unchanged); the dashboard's `receivables_outstanding` is Σ positive balances only — an account paid in advance is a liability, not a receivable, and is not netted against the ones who owe; the dashboard's "top products" card is always the last 30 days, independent of any range picker; the tax report's fiscal-status column is deferred to Phase 7 (nothing to show until invoices fiscalise). |
