...
| hourly | hour-of-day buckets (business tz) |
...
| **4 Till UI + printing** | `/pos` with WeightPad (4 modes), cart rail, customer picker, tender dialog, day-gate banners; thermal receipt + A4 invoice; reprint; kiosk-printing launcher | §6.2, §8.3, §8.4. **DONE 2026-09-20** on `dev`, plan `docs/superpowers/plans/2026-09-20-demo-path-phases-2-4.md` (tasks D7–D9). Deviations: printing is browser `window.print()` only — nothing has been through a physical printer yet; the A4 invoice's buyer address is blank unless a caller passes the `Customer` explicitly (the till's reprint path doesn't yet); counter staff can open/close the day without a PIN, per spec §3 — not yet confirmed with the owner. |
| **5 Day close UI, customers UI, dashboard, reports** | `/day-close` (open, movements, close, reopen, Z-report), `/customers` (statement, receive payment), dashboard, report tabs, exports | §6.5, §6.7, §6.8 |
