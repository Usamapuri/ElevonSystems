...
describe('toLineInput — gross − tare mode', () => {
  ...
  it('rejects gross equal to or below tare', () => {
    expect(err(toLineInput('gross_tare', { gross: '12', tare: '12' }, 265))).toMatch(/more than before-fill/)
    expect(err(toLineInput('gross_tare', { gross: '10', tare: '12' }, 265))).toMatch(/more than before-fill/)
  })
  it('rejects a missing or negative weight', () => {
    expect(err(toLineInput('gross_tare', { tare: '12' }, 265))).toMatch(/after-fill weight/)
    expect(err(toLineInput('gross_tare', { gross: '50' }, 265))).toMatch(/before-fill weight/)
...
describe('modeLabel', () => {
  it('names every mode', () => {
    ...
    expect(modeLabel('gross_tare')).toBe('before / after')
  })
})
